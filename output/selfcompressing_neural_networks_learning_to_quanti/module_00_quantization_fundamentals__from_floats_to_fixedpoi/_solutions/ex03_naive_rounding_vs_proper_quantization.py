...
====================================================
Module 0: Quantization Fundamentals  [SOLUTION]
...
At low bit-widths (1-4 bits), naive rounding fails badly because the uniform
grid spanning [min, max] wastes most of its precision on the tails of the
weight distribution, where there are very few actual weights.

Kaiming UNIFORM gives a flat distribution that suits naive well. To see the
real difference, we use weights from a TRAINED network (Gaussian-like,
concentrated near zero) and separately use a layer with Kaiming NORMAL init.

Your tasks:
  1. naive_quantize(weights, num_bits) — uniform grid over [min, max]
  2. evaluate_both(weights, bit_widths) — compare MSE of both approaches
...
def make_gaussian_weights(n_weights: int = 800, std: float = 0.08,
    ...
    """
    Generate Gaussian-distributed weights (typical of trained networks).

    Trained network weights follow an approximately Gaussian distribution
    concentrated near zero — very different from Kaiming UNIFORM init.
    This distribution reveals the advantage of proper quantization over naive.
    """
    torch.manual_seed(seed)
    weights = torch.randn(n_weights) * std
    # Add a few outliers (~2%) to simulate heavy tails in trained networks
    n_outliers = int(0.02 * n_weights)
    outlier_idx = torch.randperm(n_weights)[:n_outliers]
    weights[outlier_idx] *= 5.0  # 5x larger weights
    return weights


# ---------------------------------------------------------------------------
# Provided: a pretrained 2-layer MLP trained on random data (for comparison)
# ---------------------------------------------------------------------------
...
def train_simple_mlp(seed: int = 0) -> tuple:
    """Train a small MLP on a synthetic regression task, return (model, X_test, y_test)."""
    torch.manual_seed(seed)
    model = SimpleMLP()
...
def naive_quantize(weights: torch.Tensor, num_bits: int) -> torch.Tensor:
    ...
    normalized = (weights - w_min) / step
    indices = normalized.round().clamp(0, num_levels - 1)
...
def evaluate_both(
    ...
    For each bit-width:
      - naive MSE: apply naive_quantize, measure reconstruction MSE
      - proper MSE: find optimal exponent, apply proper quantize/dequantize
      - mse_ratio: naive_mse / proper_mse

    Parameters
    ----------
    weights : torch.Tensor
        Weight tensor to analyze (1D or multi-dimensional).
    ...
    for b in bit_widths:
        ...
        naive_mse = ((w_flat - w_naive) ** 2).mean().item()

        # --- Proper approach ---
        e_opt = find_optimal_exponent(w_flat, b)
        q = quantize(w_flat, b, e_opt)
        w_proper = dequantize(q, e_opt)
        proper_mse = ((w_flat - w_proper) ** 2).mean().item()

        # --- Ratio ---
        if proper_mse < 1e-12:
            ratio = float("inf")
        else:
            ratio = naive_mse / proper_mse

        results[b] = {
            "naive_mse": naive_mse,
            "proper_mse": proper_mse,
            "mse_ratio": ratio,
        }
...
if __name__ == "__main__":
    ...
    # --- Part A: Gaussian weights (Gaussian = heavy center, sparse tails) ---
    ...
    # Inspect actual trained weight distribution
    all_weights = []
    ...
    # Show bucket utilization for naive
    ...
    # Show bucket utilization for proper
