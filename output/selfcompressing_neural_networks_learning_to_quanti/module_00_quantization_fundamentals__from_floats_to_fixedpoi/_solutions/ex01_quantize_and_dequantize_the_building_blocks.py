...
# ---------------------------------------------------------------------------
# SOLUTION — quantize and dequantize
# ---------------------------------------------------------------------------
...
def quantize(x: torch.Tensor, num_bits: int, exponent: float) -> torch.Tensor:
    ...
    tensor([ 0., -1.,  2.])
    """
    # Signed b-bit integer range: [-2^(b-1), 2^(b-1)-1]
    q_min = -(2 ** (num_bits - 1))        # e.g., -128 for 8-bit
    q_max = 2 ** (num_bits - 1) - 1       # e.g.,  127 for 8-bit (NOT 128!)

    # Step 1: Scale into integer space (divide by step size 2^e)
    x_scaled = (2 ** (-exponent)) * x

    # Step 2: Clamp to the representable range
    x_clamped = x_scaled.clamp(q_min, q_max)

    # Step 3: Round to nearest integer (the actual discretization)
...
def dequantize(qx: torch.Tensor, exponent: float) -> torch.Tensor:
    ...
    """
    # Multiply by step size to restore original scale
