...
Usage
-----
    python ex01_uniform_scalar_quantizer.py
"""
...
def uniform_quantize(x: np.ndarray, b: int) -> np.ndarray:
    ...
    -----
    Common mistakes:
    - Off-by-one: the last bucket must be closed on the right to handle x = 1.0
      exactly.  Use np.clip after computing indices.
    - Centroids vs boundaries: this function returns INDICES, not centroid values.
      The centroid computation belongs in uniform_dequantize.
    """
    ###########################################################
    # YOUR CODE HERE - 5-7 lines                              #
    #                                                         #
    # Steps:                                                  #
    # 1. Compute the bucket width: delta = 2.0 / 2^b         #
    # 2. Shift x from [-1,1] to [0, 2] by adding 1.0        #
    # 3. Divide by delta and take the floor to get indices    #
    # 4. Cast to int                                          #
    # 5. Clip to [0, 2^b - 1] to handle boundary x = 1.0    #
    ###########################################################
...
def uniform_dequantize(indices: np.ndarray, b: int) -> np.ndarray:
    ...
    Notes
    -----
    The centroid is NOT the bucket boundary.  For b=1 (2 buckets of width 1):
        index 0 → centroid at -0.5
        index 1 → centroid at +0.5
    A common mistake is to map index 0 → -1 and index 1 → 0 (the left edges).
    """
    ###########################################################
    # YOUR CODE HERE - 3-5 lines                              #
    #                                                         #
    # Steps:                                                  #
    # 1. Compute bucket width delta = 2.0 / 2^b              #
    # 2. Centroid of bucket k = -1.0 + (k + 0.5) * delta    #
    ###########################################################
    raise NotImplementedError("YOUR CODE HERE")
    ###########################################################
...
def measure_mse(x: np.ndarray, x_hat: np.ndarray) -> float:
    ...
    Notes
    -----
    MSE here means the TOTAL squared L2 error per vector (summed over all d
    coordinates), then averaged over the n vectors.  It does NOT divide by d.
    This matches the paper's definition: D_mse = E[||x - x_hat||_2^2].

    For reference:
      - Perfect reconstruction: MSE = 0.0
      - Zeroing out x_hat:       MSE = ||x||^2 = 1.0  (for unit vectors)
      - x_hat = -x:              MSE = 4.0             (opposite direction)
    """
    ###########################################################
    # YOUR CODE HERE - 2-3 lines                              #
    #                                                         #
    # Hint: compute the squared difference per element,       #
    # sum over the d coordinates per vector (axis=1),         #
    # then average over n vectors.                            #
    ###########################################################
    raise NotImplementedError("YOUR CODE HERE")
    ###########################################################
...
if __name__ == "__main__":
    ...
    print(f"  only ~{1 - 0.951:.3f} of data lives there.")
    print()
