"""
Exercise 3: Concentration and Near-Independence in High Dimensions
==================================================================
...
def compute_gaussian_approx_error(samples, d):
    ...
    #                                                                         #
    # Step 1: Create x_grid = np.linspace(-4/sqrt(d), 4/sqrt(d), 500)       #
    # Step 2: Compute empirical CDF on x_grid using broadcasting:            #
    #         ecdf = (samples[:, None] <= x_grid[None, :]).mean(axis=0)      #
    # Step 3: Compute Gaussian N(0, 1/d) CDF:                               #
    #         gauss_cdf = scipy_norm.cdf(x_grid, loc=0, scale=1/sqrt(d))     #
    # Step 4: Return float(np.max(np.abs(ecdf - gauss_cdf)))                 #
    ###########################################################################
...
def compute_coordinate_correlation(vectors, Pi, i, j):
    ...
    Notes
    -----
    Rotate: rotated = Pi @ vectors.T   # shape (d, n)
    Extract: coord_i = rotated[i, :], coord_j = rotated[j, :]
    Correlation: np.corrcoef(coord_i, coord_j)[0, 1]
    """
    ###########################################################################
    # YOUR CODE HERE - 4-6 lines                                              #
    #                                                                         #
    # Step 1: Rotate all vectors at once: rotated = Pi @ vectors.T           #
    # Step 2: Extract rows i and j: coord_i = rotated[i, :]                  #
    # Step 3: Compute Pearson correlation using np.corrcoef                   #
    # Step 4: Return the [0, 1] element (off-diagonal entry)                 #
    ###########################################################################
    raise NotImplementedError("YOUR CODE HERE")
    ###########################################################################
...
def compute_effective_variance_ratio(samples, d):
    ...
    Notes
    -----
    This is a simple one-liner but an important sanity check:
    the Beta distribution f_X has EXACTLY variance 1/d for all d ≥ 2.
    """
    ###########################################################################
    # YOUR CODE HERE - 3-4 lines                                              #
    #                                                                         #
    # Compute: empirical_var = np.var(samples)                               #
    # Compute: theoretical_var = 1.0 / d                                     #
