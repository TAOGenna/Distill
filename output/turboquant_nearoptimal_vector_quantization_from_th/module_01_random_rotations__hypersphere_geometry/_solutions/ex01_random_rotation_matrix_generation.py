"""
Exercise 1: Random Rotation Matrix Generation  [SOLUTION]
=========================================================
...
def generate_random_rotation(d, seed=None):
    ...
    # Step 1: Draw d×d matrix with i.i.d. N(0,1) entries
    G = rng.standard_normal((d, d))
    # Step 2: QR decomposition — G = Q · R where Q orthogonal, R upper triangular
    Q, R = np.linalg.qr(G)
    # Step 3: Sign correction to enforce positive diagonal of R (Haar uniformity)
    # np.diag(R) extracts the diagonal; np.sign gives +1 or -1 per element
    signs = np.sign(np.diag(R))
    # Multiply each column j of Q by signs[j]
    # signs has shape (d,); use newaxis to broadcast over rows: shape (1, d)
...
if __name__ == "__main__":
    ...
    print("\n--- KS test: rotated coordinates vs N(0, 1/d) at d=128 ---")
    from scipy.stats import kstest, norm as scipy_norm
    d_ks = 128
    Pi_ks = generate_random_rotation(d_ks, seed=42)
    rng_ks = np.random.default_rng(7)
    G_ks = rng_ks.standard_normal((10000, d_ks))
    unit_vecs = G_ks / np.linalg.norm(G_ks, axis=1, keepdims=True)
    coord_samples = (Pi_ks @ unit_vecs.T)[0, :]
    ks_stat, ks_pval = kstest(coord_samples,
                               lambda x: scipy_norm.cdf(x, loc=0, scale=1.0/np.sqrt(d_ks)))
    print(f"KS test vs N(0, 1/d): stat={ks_stat:.4f}, p-value={ks_pval:.4f}")
    print(f"p > 0.05 confirms rotated coordinates follow the Gaussian approximation")

    print("\n--- Cross-coordinate correlation check ---")
    rotated_all = (Pi_ks @ unit_vecs.T).T   # shape: (10000, d)
    coord_pairs = [(0, 1), (0, 2), (1, 3), (2, 5), (10, 20)]
    corr_vals = [abs(np.corrcoef(rotated_all[:, i], rotated_all[:, j])[0, 1])
                 for i, j in coord_pairs]
    print(f"Mean |cross-coordinate correlation| = {np.mean(corr_vals):.4f}  (should be < 0.02)")
    print(f"Low correlation confirms coordinates are nearly independent after rotation")
