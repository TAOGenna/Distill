...
def quantize_database(quantizer, database):
    ...
    quantized = []
    for x in database:
        idx, qjl_bits, gamma = quantizer.quantize(x)
        quantized.append((idx, qjl_bits, gamma))
    return quantized
...
def approximate_topk(quantizer, quantized_db, query, k):
    ...
    n = len(quantized_db)
    scores = np.empty(n)
    for j, (idx, qjl_bits, gamma) in enumerate(quantized_db):
        x_approx = quantizer.dequantize(idx, qjl_bits, gamma)
        scores[j] = float(query @ x_approx)
...
def compute_recall_at_k(true_topk, approx_topk, k):
    ...
    """
    true_set = set(np.asarray(true_topk).tolist())
...
def run_nn_experiment(quantizer_class, database, queries, k_values, b, d):
    ...
    """
    quantizer = quantizer_class(d=d, b=b, seed=42)

    # Step 1: Indexing (offline quantization — timed separately)
    t0 = time.perf_counter()
    quantized_db = quantize_database(quantizer, database)
    index_time = time.perf_counter() - t0

    # Step 2: Ground truth
    max_k = max(k_values)
    true_topk_all = compute_exact_topk(queries, database, max_k)  # (n_q, max_k)

    # Step 3: Query loop
    recalls = {k: [] for k in k_values}
    t0 = time.perf_counter()
    for qi, q in enumerate(queries):
        approx_idx = approximate_topk(quantizer, quantized_db, q, max_k)
        for k in k_values:
            r = compute_recall_at_k(true_topk_all[qi, :k], approx_idx, k)
            recalls[k].append(r)
...
if __name__ == "__main__":
    ...
    print()
    print("  Memory compression: b=4 → 4.0× compression vs FP16,")
    print("  b=3 → 5.3×, b=2 → 8.0×. High recall is maintained even at")
    print("  substantial compression levels, confirming near-optimality.")
    print("DONE — nearest neighbor search experiment complete.")
