...
def compute_theoretical_upper_bound(b):
    ...
    """
    # Exact values from Lloyd-Max solver (tighter than Panter-Dite for small b)
    exact = {1: 0.3634, 2: 0.1175, 3: 0.0345, 4: 0.0095}
    if b in exact:
        return exact[b]
    # Panter-Dite asymptotic formula for b > 4
...
def run_distortion_experiment(d, b, n_vectors=8000, seed=0):
    ...
    qmse = TurboQuantMSE(d=d, b=b, seed=seed + 1)
    indices = qmse.quantize_batch(X)
    X_hat = qmse.dequantize_batch(indices)
    residuals = X - X_hat
    per_vec_mse = np.sum(residuals ** 2, axis=1)
    return float(np.mean(per_vec_mse))
...
def compute_ratio_to_optimal(empirical_mse, lower_bound):
    ...
    if lower_bound == 0.0:
        return float('inf')
    return empirical_mse / lower_bound
...
if __name__ == "__main__":
    ...
    print("  (D_mse reported = total MSE; per-coordinate MSE = D_mse / d)")
    print()
