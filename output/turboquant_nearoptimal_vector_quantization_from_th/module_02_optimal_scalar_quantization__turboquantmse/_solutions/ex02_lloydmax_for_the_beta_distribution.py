...
def beta_conditional_expectation(a, b_bound, d):
    ...
    """
    # Clip bounds to the support [-1, 1]
    a_clip = max(a, -1.0)
    b_clip = min(b_bound, 1.0)

    if a_clip >= b_clip:
        return (a_clip + b_clip) / 2.0

    numerator, _ = integrate.quad(
        lambda x: x * beta_pdf(x, d), a_clip, b_clip
    )
    denominator, _ = integrate.quad(
        lambda x: beta_pdf(x, d), a_clip, b_clip
    )
...
def update_centroids_beta(boundaries, d, n_buckets):
    ...
    for i in range(n_buckets):
        new_centroids[i] = beta_conditional_expectation(
...
def compute_per_coordinate_mse(centroids, boundaries, d):
    ...
    for i, ci in enumerate(centroids):
        a_clip = max(boundaries[i], -1.0)
        b_clip = min(boundaries[i + 1], 1.0)
        if a_clip >= b_clip:
            continue
        cost_i, _ = integrate.quad(
            lambda x, c=ci: (x - c) ** 2 * beta_pdf(x, d),
            a_clip, b_clip
        )
        total_cost += cost_i
    return total_cost
...
if __name__ == "__main__":
    ...
    # Note: the paper rounds b=3 to 0.03; the true Gaussian-optimal value
    # is 0.0345. We use the true values here to avoid false FAIL.
    TARGET_MSE = {1: 0.363, 2: 0.117, 3: 0.0345, 4: 0.0095}
    ...
    for b in [1, 2, 3, 4]:
        ...
        print(f"  per-coordinate MSE = {per_coord_mse:.6f}   (theoretical: {TARGET_MSE[b]/d:.6f})")
    ...
    print("=" * 65)
    PAPER_TARGET = {1: 0.36, 2: 0.117, 3: 0.030, 4: 0.009}
    print(f"{'b':>3}  {'C(f_X,b)':>12}  {'per-coordinate MSE':>20}  {'paper target':>14}")
    for b in [1, 2, 3, 4]:
        pdf_func = lambda x, dim=d: beta_pdf(x, dim)
        init_c = initial_centroids_symmetric(b, d)
        update_fn = lambda bounds, bits, dim=d: update_centroids_beta(
        ...
        print(f"  {b:1d}  {c_cost:12.6f}  {d*c_cost:>20.4f}  "
              f"{PAPER_TARGET[b]:>14.4f}")
