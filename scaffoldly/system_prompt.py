...
- Sub-agents: `reviewer` — adversarial reviewer that audits generated files

Module generation is handled automatically by the orchestrator after you \
finish steps 1-4. You do NOT need to dispatch module generators yourself.
...
cost figures) from the source material. These will be passed to module \
generators as milestone targets.
   • After creating root files, STOP. You will be prompted again after modules \
are generated.

5. REVIEW (adversarial quality check)
   • You will be told when all modules are generated.
   • Dispatch the `reviewer` sub-agent to audit the generated files.
   • If the reviewer says REVISE, fix the specific issues and re-review.
...
# Used by the orchestrator for programmatic parallel dispatch.
# Includes all pedagogy guidelines so generators are self-sufficient.

MODULE_GENERATOR_SYSTEM_PROMPT = """\
...
not recall). See the question rubric below.
...
exercise type conventions below.
